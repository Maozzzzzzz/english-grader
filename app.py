            ...
            if not api_key:
                ...
                try:
                    with st.spinner("正在絞盡腦汁出題中..."):
                        ...
                        """
                        # 使用智慧函數
                        result = get_gemini_response(prompt_gen, api_key)
                        st.session_state.generated_topic = result
                        
                except Exception as e:
                    st.error(f"出題錯誤：{e}")
                    st.warning("請檢查 API Key 是否正確複製，或者該 Key 是否已啟用。")
            ...
            try:
                ...
                **1. [分類]**
                ...
                with st.spinner("AI 名師正在嘗試連接大腦並批改中..."):
                    # 使用智慧函數
                    result = get_gemini_response(system_prompt, api_key)
                    st.markdown(result)
                    
            except Exception as e:
                st.error(f"批改失敗：{e}")
                st.error("建議：請確認 API Key 是否有複製到多餘的空白，或者嘗試重新整理網頁。")